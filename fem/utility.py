...
class StructureUtils:
    @staticmethod
    def get_perimeter_nodes(structure, geometry):
        if geometry in ('quadrilateral', 'square'):
            corners = {
                'down_left':  lambda x: np.isclose(x[0], structure.x_min) and np.isclose(x[1], structure.y_min),
                'down_right': lambda x: np.isclose(x[0], structure.x_max) and np.isclose(x[1], structure.y_min),
                'up_left':    lambda x: np.isclose(x[0], structure.x_min) and np.isclose(x[1], structure.y_max),
                'up_right':   lambda x: np.isclose(x[0], structure.x_max) and np.isclose(x[1], structure.y_max)
            }

            lines = {
                'bottom': lambda x: np.isclose(x[1], structure.y_min) and structure.x_min < x[0] < structure.x_max,
                'top':    lambda x: np.isclose(x[1], structure.y_max) and structure.x_min < x[0] < structure.x_max,
                'left':   lambda x: np.isclose(x[0], structure.x_min) and structure.y_min < x[1] < structure.y_max,
                'right':  lambda x: np.isclose(x[0], structure.x_max) and structure.y_min < x[1] < structure.y_max
            }
