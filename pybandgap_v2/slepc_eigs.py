...
def calc_eigs(A, B=None, n_eigs=6, which="LM", tol=1e-10):
    """
...
def normalize_eigenvectors_B(eigenvects, B):
    ...
    Corregido: Ajuste de tolerancia y manejo de precisión numérica.
    """
    B = B[:, :]
    ...
    # Verificar ortonormalidad con tolerancia relajada
    for i in range(n_eigs):
        for j in range(n_eigs):
            dot_product = np.conj(normalized_eigenvects[:, i]).T @ B @ normalized_eigenvects[:, j]
            if i == j:
                if not np.isclose(dot_product, 1.0, atol=1e-6, rtol=1e-6):
                    normalized_eigenvects[:, i] /= np.sqrt(dot_product)
            else:
                if not np.isclose(dot_product, 0.0, atol=1e-6, rtol=1e-6):
                    # Re-ortogonalizar si es necesario
                    normalized_eigenvects[:, j] -= dot_product * normalized_eigenvects[:, i]
...
def gram_schmidt_B(vectors, B):
    ...
    for i in range(n_vectors):
        ...
        if norm_B < 1e-12:  # Verificar si el vector se anuló numéricamente
            raise ValueError(
                f"El vector {i} es linealmente dependiente de los anteriores."
            )
